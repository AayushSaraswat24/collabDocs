...
export async function POST(request:NextRequest) {
  ...
  try{
    ...
    const prompt=`Rewrite the following text with correct grammar and punctuation:\n\n${documentText}`;
    ...
    const geminiStream = await gemini.models.generateContentStream({
      ...
      config:{
        ...
        Your task is to rewrite the provided text with correct grammar, spelling, punctuation, and sentence structure while preserving the original meaning.
        Rules:
        - Return ONLY the corrected text.
        - Do NOT include explanations, comments, or labels.
        - Do NOT add new information or change the meaning.
        - Do NOT write phrases like "Corrected text:".
